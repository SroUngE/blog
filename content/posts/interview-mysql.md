...
### InnoDB 的 redo log
redo log主要用于确保事务的持久性。该日志文件由两部分组成：redo log buffer（内存） 和 redo log file（磁盘）。当事务提交时，InnoDB会先将修改的信息记录到redo log中，而不是直接写入数据文件。这是因为直接写入数据文件是一个随机IO操作，效率较低，而写入redo log是一个顺序IO操作，效率更高。当系统崩溃或重启时，InnoDB可以通过redo log来恢复未提交到数据文件的事务，从而确保数据的完整性。

redo log是循环写入的，当日志空间被写满时，InnoDB会覆盖最旧的日志记录。但是，由于InnoDB使用了多版本并发控制（MVCC）和undo log等技术，它可以在不覆盖任何未提交事务的redo log记录的情况下进行日志的循环写入。

### InnoDB 的 undo log
undo log主要用于实现事务的原子性。在事务执行过程中，InnoDB会生成undo log来记录数据修改前的状态。如果事务在执行过程中发生错误或需要回滚，InnoDB可以利用undo log将数据恢复到事务开始前的状态。


### InnoDB 的事务

InnoDB存储引擎具有ACID属性，即原子性（Atomicity）、一致性（Consistency）、隔离性（Isolation）和持久性（Durability）。

1. **原子性（Atomicity）：**
   - **定义：** 事务是一个原子操作单元，要么全部执行成功，要么全部回滚到事务开始前的状态。
   - **实现：** InnoDB通过 Undo Log 记录修改前的数据状态，以便数据回滚。

2. **一致性（Consistency）：**
   - **定义：** 在事务开始和结束时，数据库必须保持一致的状态。如果事务执行成功，数据库应该从一个一致的状态转移到另一个一致的状态。
   - **实现：** InnoDB通过 MVCC 确保每个事务在读取数据时都会看到一个一致的数据快照，即使其他事务正在修改数据。

3. **隔离性（Isolation）：**
   - **定义：** 各个事务的执行应该相互隔离，一个事务的执行不应该影响其他事务的执行。
   - **实现：** InnoDB使用锁机制来确保事务在并发执行时的隔离性。它支持行级锁和表级锁，其中行级锁是InnoDB的主要锁定机制。

4. **持久性（Durability）：**
   - **定义：** 一旦事务被提交，其结果应该永久保存在数据库中，即使发生系统故障。
   - **实现：** InnoDB通过 Redo日志 记录修改数据，以便数据恢复。
